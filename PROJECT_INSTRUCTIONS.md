...
이 프로젝트는 Flask의 핵심 개념인 **Routing**을 명확하게 이해하고 일관된 코드 스타일을 유지하기 위해 `@app.route()`를 기본 라우팅 방식으로 사용한다.

원칙:

```text
모든 라우팅은 @app.route()를 기본으로 사용한다.
GET 요청이라도 @app.get() 대신 @app.route()를 사용한다.
POST, PUT, PATCH, DELETE도 methods 인자를 사용하여 명시한다.
동일한 URL에서 여러 HTTP Method를 처리하는 경우 하나의 route에서 관리한다.
```
...
```text
Flask의 핵심 개념인 Routing을 코드에서 명확하게 표현하기 위함.
HTTP Method가 추가되더라도 동일한 문법을 유지하기 위함.
교육 자료와 프로젝트 간 코드 스타일을 일관되게 유지하기 위함.
Flask 공식 문서에서 가장 널리 사용되는 기본 문법을 따른다.
```

`@app.get()`, `@app.post()` 등은 Flask에서 제공하는 편의(축약) 문법으로 이해하며, 본 프로젝트에서는 특별한 사유가 없는 한 사용하지 않는다.
...
## 19-2. Git 작업 흐름과 커밋 원칙

Git 기록은 단순 저장 지점이 아니라 개발 과정과 판단을 남기는 학습 자료로 본다.
...
```text
한국어 커밋 메시지를 허용하고 권장한다.
영어 type prefix와 한국어 설명을 함께 사용한다.
Phase prefix를 붙여 작업 흐름을 추적한다.
Codex가 만든 변경도 사용자가 git diff를 검토한 뒤 커밋한다.
승인 없는 Codex commit/push는 금지한다.
기능 추가와 refactor를 한 커밋에 섞지 않는다.
보안/권한 정책 변경은 커밋 메시지에 명확히 드러낸다.
```
